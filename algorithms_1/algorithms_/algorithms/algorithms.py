...
def genetic_alg(input_data, tournament = 0):
    ...
    def parents_tournament():
        half_size = len(population) // 2
        first_half = population[:half_size]
        second_half = population[half_size:]

        # Знаходимо найкраще значення в кожній половині
        p1_index, p1 = max_result(first_half)
        p2_index, p2 = max_result(second_half)
        return p1,p2
    ...
    for i in range(iterations):
        if (tournament):
            p1, p2 = parents_tournament()
        else:
            p1, p2 = parents_current()
